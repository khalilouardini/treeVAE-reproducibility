...
class PPCA:
    def __init__(self,
        ...
        self.leaves = [n for n in self.tree.traverse('levelorder') if n.is_leaf()]
        self.internal_nodes = [n for n in self.tree.traverse('levelorder') if not n.is_leaf()]

        self.n_leaves = len(self.leaves)
        self.n_nodes = len(self.leaves) + len(self.internal_nodes)
        ...
        leaves_covariance, full_covariance = precision_matrix(self.tree,
                                                              self.latent,
                                                              self.branch_length
                                                             )
        if self.leaves_only:
    ...
    def likelihood_obs(self, leaves_only=True):
        ...
        for n in self.tree.traverse('levelorder'):
            ...
            log_likelihood_x += multivariate_normal.logpdf(evidence,
                                            np.array([0] * self.dim),
                                            cov
                                            )
    ...
    def compute_leaves_covariance(self):
        ...
        for i1, n1 in zip(indices, self.leaves):
            for i2, n2 in zip(indices, self.leaves):
        ...
        self.leaves_covariance_x += self.eps * np.identity(self.n_leaves * self.dim)
    ...
    def compute_posterior(self):
        ...
        for n1 in self.leaves:
            ...
            posterior_cov[n1.index] += self.eps * np.identity(self.latent)
    ...
    def compute_correlated_posterior(self):
        ...
        for i1, n1 in zip(indices, self.leaves):
            for i2, n2 in zip(indices, self.leaves):
                sigma_n1_n2 = self.query_marginalized_covariance_z(idx1=n1.index, idx2=n2.index)
                start1, end1 = i1 * self.latent, i1 * self.latent + self.latent
                start2, end2 = i2 * self.dim, i2 * self.dim + self.dim       
                corr_x_z[start1: end1, start2: end2] = sigma_n1_n2 @ self.W.T
        ...
        posterior_cov += self.eps * np.identity(self.n_leaves * self.latent)
    ...
    def compute_posterior_predictive(self):
        ...
        # We want to compute the posterior of n1 condtioned on all the leaves
        for n1 in self.internal_nodes:
            # Correlations between x_n and x_L
            ...
            predictive_cov[n1.name] += self.eps * np.identity(self.dim)
    ...
    def compute_posterior_predictive_z_MP(self, evidence):
        ...
        gene_dataset = AnnDatasetFromAnnData(ad=adata,
                                            filtering=False)
        barcodes = [l.name for l in self.leaves]
        gene_dataset.initialize_cell_attribute('barcodes', barcodes)

        #create tree dataset
        cas_dataset = TreeDataset(gene_dataset,
                                 tree=self.tree,
                                  filtering=False)
    ...
    def compute_approx_posterior_predictive(self, iid=False, use_MP=False, sample_size=100):
        ...
        if not iid:
            ...
            for k in list(posterior_mean.keys()):
                samples_z = np.array([np.random.multivariate_normal(mean=posterior_mean[k],
